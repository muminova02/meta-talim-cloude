...
const Navigation = () => {
  ...
  const [isUniversitetDropdownOpen, setIsUniversitetDropdownOpen] = useState(false)
  const [isOqituvchilarDropdownOpen, setIsOqituvchilarDropdownOpen] = useState(false)
  ...
  const maktabHandlers = createDropdownHandlers(setIsMaktabDropdownOpen, isMaktabDropdownOpen)
  const mahsulotlarHandlers = createDropdownHandlers(setIsMahsulotlarDropdownOpen, isMahsulotlarDropdownOpen)
  const universitetHandlers = createDropdownHandlers(setIsUniversitetDropdownOpen, isUniversitetDropdownOpen)
  const oqutuvchilarHandlers = createDropdownHandlers(setIsOqituvchilarDropdownOpen, isOqituvchilarDropdownOpen)
  const yordamHandlers = createDropdownHandlers(setIsYordamDropdownOpen, isYordamDropdownOpen)
    ...
    <div className="relative" onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
      ...
      {isOpen && (
        <>
          ...
          >
            <div className="py-2">
              <div className="px-4 py-2 text-gray-700 font-semibold border-b border-gray-100">{title}</div>
            ...
            <div className="flex flex-col space-y-4">
              ...
              <div className="space-y-2">
                ...
                >
                  Maktab
                  <ChevronDown
                    className={`h-4 w-4 transition-transform duration-200 ${isMaktabDropdownOpen ? "rotate-180" : ""}`}
                  />
                </button>
                {isMaktabDropdownOpen && (
                  <motion.div
              ...
              <div className="space-y-2">
                ...
                {isMahsulotlarDropdownOpen && (
                  <motion.div
                    ...
                    className="pl-4 space-y-1"
                  >
                    {["Kitoblar", "Video darslar", "Test topshiriqlari", "Interaktiv o'yinlar"].map((item, index) => (
                      <a
                        key={index}
                        href="#"
                        className="block text-emerald-200 hover:text-white transition-colors duration-200 py-1 text-sm pl-2"
              ...
              {navItems.map((item, index) => (
                <div key={index} className="space-y-2">
                  <div className="text-white font-medium py-2 border-b border-emerald-400">{item.name}</div>
